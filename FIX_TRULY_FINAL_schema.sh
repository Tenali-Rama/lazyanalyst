...
def detect(df):
    ...
    for col in df.columns:
        ...
        # check column name keywords for identifier
        is_identifier = False
        ...
        # check boolean first (2 unique values with bool-like values)
        if unique_vals == 2:
        ...
        # check categorical - string type with < 20 unique OR all unique strings
        if df[col].dtype == 'object' or str(df[col].dtype).startswith('string'):
            # if fewer than 20 unique values, definitely categorical
            if unique_vals < 20:
                schema[col] = 'categorical'
                print(f"  {col} -> categorical")
                continue
            # if all unique strings (not just < 20), then identifier
